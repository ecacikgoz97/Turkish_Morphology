...
class CharLM(nn.Module):
    ...
    def __init__(self, args, vocab, model_init, emb_init, bidirectional=False):
        ...
        self.lstm = nn.LSTM(input_size=args.ni, hidden_size=args.enc_nh, num_layers=1, batch_first=True, dropout=0, bidirectional=bidirectional)

        self.dropout_in = nn.Dropout(args.enc_dropout_in)
        self.dropout_out = nn.Dropout(args.enc_dropout_out)

        # prediction layer
        self.pred_linear = nn.Linear(args.enc_nh, len(vocab.word2id), bias=False)
